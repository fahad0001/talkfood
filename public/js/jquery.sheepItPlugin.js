        ...
        function clickOnRemoveCurrent(event)
        {
            ...
            if (options.removeCurrentConfirmation) {
                if ( confirm(options.removeCurrentConfirmationMsg) ) {
                    removeCurrentForm($j(this).data('removableClone'));
                }
            } else {
                removeCurrentForm($j(this).data('removableClone'));
            }
...
var nameTemplateAttr = that[0]['name'];
                ...
                if (options.nestedForms.length > 0) {
                    ...
                    for(x in options.nestedForms) {
                        ...
                        if (typeof(options.nestedForms[x].id) != 'undefined' && typeof(options.nestedForms[x].options) != 'undefined') {
                            ...
                            var nestedForm = $j('#' + id).sheepIt(options.nestedForms[x].options);
        ...
        function fillFormField(field, value)
        {
            ...
            // select-one, select-multiple
                ...
                field.find("option").each(function() {
                    if($j(this).text() == value || $j(this).attr("value") == value) {
        ...
        {
            options = [];
            options = $j.extend(defaults, newOptions);
        ...
        function initialize()
        {
             ...
             * Controls
             */
            add = $j(options.addSelector);
            addN = $j(options.addNSelector);
            addNInput = $j(options.addNInputSelector);
            addNButton = $j(options.addNButtonSelector);
            removeLast = $j(options.removeLastSelector);
            removeCurrent = $j(options.removeCurrentSelector);
            removeAll = $j(options.removeAllSelector);
            controls = $j(options.controlsSelector);
             ...
             * Templates
             */
            templateForm = $j(options.formTemplateSelector);
            noFormsTemplate = $j(options.noFormsTemplateSelector);
        ...
        function extendSource(source)
        {
            ...
            $j.extend( source, {
                    ...
                    inject: function(data) {
                        ...
                        $j.each(data, $j.proxy( fillData, source ));
                    }
        ...
        var add,
            ...
            defaults = {
                ...
                // Controls selectors
                addSelector: '#' + $j(this).attr("id") + '_add',
                addNSelector: '#' + $j(this).attr("id") + '_add_n',
                addNInputSelector: '#' + $j(this).attr("id") + '_add_n_input',
                addNButtonSelector: '#' + $j(this).attr("id") + '_add_n_button',
                removeLastSelector: '#' + $j(this).attr("id") + '_remove_last',
                removeCurrentSelector: '#' + $j(this).attr("id") + '_remove_current',
                removeAllSelector: '#' + $j(this).attr("id") + '_remove_all',
                controlsSelector: '#' + $j(this).attr("id") + '_controls',
                labelSelector: '#' + $j(this).attr("id") + '_label',
                ...
                // Templates
                formTemplateSelector: '#' + $j(this).attr("id") + '_template',
                noFormsTemplateSelector: '#' + $j(this).attr("id") + '_noforms_template',
    ...
    jQuery.fn.cloneWithAttribut = function( withDataAndEvents ){
        if ( jQuery.support.noCloneEvent ){
            return $j(this).clone(withDataAndEvents);
        }else{
            $j(this).find("*").each(function(){
                $j(this).data("name", $j(this).attr("name"));
            });
            var clone = $j(this).clone(withDataAndEvents);
